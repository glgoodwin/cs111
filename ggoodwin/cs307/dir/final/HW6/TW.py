...
def which(filename, search_path=sys.path, verbose=False):
    ...
    for directory in dirs:
        ...
        if os.path.exists(os.path.join(directory, filename)):
            if verbose:
                print "found ", os.path.join(directory, filename)
            return(directory)
    return None
...
twLoadPath = "~/cs307/dir/final/HW6"
...
'''
    ...
    filedata = FILE.read()
    if len(filedata) != width*height*3:
        print "hmm, we don't seem to have the right amount of data: %d x %d x 3 != %d" % (width, height, len(filedata))
    if verbose:
