...
public class StockInfo{
    ...
    private int signal; //signal will be known as soon as stock is created 
    private String symbol;
    private Vector<StockQuote> quotes;
   

    /** Constructor **/
    public StockInfo (String symbol){
	this.symbol = symbol;
	this.quotes = new Vector<StockQuote> ();
	String yahooString = 
	...
	signal = getSignal(this.lastDate());
	System.out.println(signal);
			   
    }
//     /**Sorts the quotes by price rather than date**/
//     private LinkedList<StockQuote> sortQuotesByPrice(){
...
//     }
	...
	if(quotes.size() == 0){
	    ...
	    return quotes.lastElement().getDate();
	}
	
    }

    /** returns the last acailable date for the stock**/
    public CS230Date lastDate(){
	if(quotes.size() == 0){
	    return CS230Date.MIN_DATE;
	}else{
	    return quotes.firstElement().getDate();
	}
    }
	...
	if((date.compareTo(firstDate())<0) || (date.compareTo(lastDate()) >0)){
	    ...
	    for(int i =0; i<quotes.size(); i++){
		if(date.compareTo(quotes.get(i).getDate()) >= 0)
		return quotes.get(i);
	    }
	}
	return null;
...
//     }
    ...
    protected static CS230Date stringToDate(String s){
	try{
...
//     }
  ...
  /**gives user feedback in the linux window**/
    ...
    //New Methods that I have written for this class
	...
	double average = (totalPrices/numStocks);

	return average;

    }

    public double getSignalLine(CS230Date date){
	double totalMACD = 0;
	int i =9;
	while(i>0){
	    double newMACD = getMACD(date);
	    totalMACD = totalMACD + newMACD;
	    date = date.prev();
	    i--;
	    }
	double sigLine = totalMACD/9;
	System.out.println("SigLine: " +Double.toString(sigLine));
	return sigLine;
    }

    public double getMACD(CS230Date date){
	double MACD = ((getAverage(date,12)) - (getAverage(date,26)));
	System.out.println("MACD: " +Double.toString(MACD));
	return MACD;
    }
	...
	int sig = 0;
	double signalLine = getSignalLine(date);
	double MACD = getMACD(date);
	if(MACD > signalLine){
	    sig = 1; //if the signal should be buy
	} else if( MACD <= signalLine ){
	    sig = -1; //if the signal should be sell
	}
 return sig;
    }
	...
	StockInfo bob = new StockInfo("GOOG");
	CS230Date date = stringToDate("2010-10-02");
	System.out.println(bob.getSignal(date));
	System.out.println(bob.getAverage(date,2));
