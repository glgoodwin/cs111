(***************************************************************
 Loopex addes the loop construct to Valex.
 ****************************************************************)

module Loopex = struct
  ...
  and exp =
    ...
    | Loop of var list (* state variable names *)
              * exp list (* initialization expressions *)
              * exp list (* update expressions *)
              * exp (* test expression *)
              * exp (* body expression *)
  ...
  let rec subst exp env = 
    ...
    | Loop(vars,inits,updates,test,body) -> 
	Loop((map (fun f -> StringUtils.fresh f) vars), (map (flip subst env) inits), (map (flip subst env) updates), subst test env, subst body env) 
 ...
 (* rename <oldNames> <newNames> <exp> renames <oldNames> to <newNames> in <exp> *)
  ...
  let rec desugar sexp = 
    let sexp' = desugarRules sexp in 
	  ...
	  Seq [Sym "loop"; Seq bindingxs; testx; bodyx] -> 
	    let (names,initxs,updatexs) = parseLoopBindings bindingxs
	    in Seq [Sym "loop"; 
		     Seq (map3 (fun n i u -> Seq [Sym n; desugar i; desugar u])
                               names initxs updatexs);
                     desugar testx;
		     desugar bodyx]
  ...
  and desugarRules sexp = 
    ...
    (* Add your SIMPREC desugaring from part 1d here *)

    | _ -> sexp

  (* For testing *)
  and desugarString str = 
  ...
  and sexpToPgm sexp = 
    match sexp with 
      Seq [Sym "loopex"; Seq formals; bodyx] -> 
  ...
  and sexpToExp' sexp = 
    ...
    | Seq [Sym "loop"; Seq bindingxs; testx; bodyx] -> 
	let (names,initxs,updatexs) = parseLoopBindings bindingxs in 
	Loop(names, map sexpToExp' initxs, map sexpToExp' updatexs, 
             sexpToExp' testx, sexpToExp' bodyx)
  ...
  and parseBindings bindingxs = 
    unzip (map (fun bindingx -> 
		 ...
		 | _ -> raise (SyntaxError ("ill-formed bindZZZ binding"
					    ^ (sexpToString bindingx)))))
             bindingxs)

  (* parse bindings of the form ((<name_1> <initx_1> <updatex_1>) ... (<name_n> <initx_n> <updatex_n>)
     into ([name_1;...;name_n], [initx_1; ...; initx_n], [updatex_1; ...; updatedx_n]) *)
  and parseLoopBindings bindingxs = 
    unzip3 (map (fun bindingx -> 
                  (match bindingx with 
		    Seq[Sym name; initx; updatex] -> (name, initx, updatex)
		  | _ -> raise (SyntaxError ("ill-formed loop binding"
			 		     ^ (sexpToString bindingx)))))
              bindingxs)
  ...
  and expToSexp e = 
    ...
    | Loop(vars,inits,updates,test,body) -> 
      Seq [Sym "loop"; 
           Seq (map (fun (name,initx,updatex) -> Seq [Sym name; initx; updatex])
                    (zip3 (vars, map expToSexp inits, map expToSexp updates)));
           expToSexp test; 
           expToSexp body]
  ...
  let testFreeVarsExp expStr = setToList (freeVarsExp (stringToExp expStr))

  let rec testSubst expStr bindingsStr = 
    StringUtils.println
      (expToString
       (subst (stringToExp expStr) 
              (parseEnvBindings (stringToSexp bindingsStr))))

  and parseEnvBindings bindingsx = 
    match bindingsx with 
      Seq bindingxs -> 
        let (names, exps) = unzip (map parseEnvBinding bindingxs)
         in Env.make names exps
    | _ -> raise (Failure ("Invalid representation of environment bindings: "
			   ^ (sexpToString bindingsx)))

  and parseEnvBinding bindingx = 
    match bindingx with 
      Seq [Sym name; expx] -> (name, sexpToExp expx)
    | _ -> raise (Failure ("Invalid representation of environment binding: "
			   ^ (sexpToString bindingx)))
