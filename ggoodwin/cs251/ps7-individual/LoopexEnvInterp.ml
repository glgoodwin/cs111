...
module LoopexEnvInterp = struct
  ...
  (* val run : Loopex.pgm -> int list -> valu *)
  ...
  (* val eval : Loopex.exp -> valu Env.env -> valu *)
  and eval exp env = 
    match exp with 
      Lit v -> v
    | Var name -> 
...
#	eval body (iterate 
		     (fun (i, u) ->((eval inits env),(eval updates env))) 
		     (eval test env) 
		     (Env.make vars))
  ...
  let repl () =
    ...
    let rec loop env = 
      ...
      let line = read_line () in 
        ...
        | _ -> 
            try 
             (print (valuToString (eval (stringToExp line) env)); 
              loop env)
             with 
