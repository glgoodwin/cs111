...
module Fobs = struct
  ...
  type pgm = Pgm of var list * exp (* formals, body *)
  ...
  and exp =
    ...
    | Funrec of exp * fcn list (* body, function declarations *)
  ...
  let primops = [ 
    ...
    Primop("error", checkTwoArgs (checkString,checkAny)
                      (fun s x -> raise (EvalError ("Fobs Error -- " ^ s ^ ":" ^ (valuToString x))))); 
  ...
  let rec freeValVarsPgm (Pgm(fmls,body)) = 
    S.diff (freeValVarsExp body)
           (listToSet fmls)
  ...
  and freeValVarsExp e = 
    ...
    | Funrec(body,fcns) -> S.union (freeValVarsExp body) (freeValVarsFcns fcns)
  ...
  (* Returns the free variables of a program *)
  let rec freeFcnVarsPgm (Pgm(fmls,body)) = freeFcnVarsExp body
  ...
  and freeFcnVarsExp e = 
    ...
    | Funrec(body,fcns) -> S.union (freeFcnVarsExp body) (freeFcnVarsFcns fcns)
  ...
  let rec subst exp env = 
    ...
    | Funrec(body, fcns) -> 
        Funrec(subst body env, map (flip substFcn env) fcns)

  and substFcn (Fcn(fname, fmls, body)) env = 
    let fmls' = map StringUtils.fresh fmls 
     in Fcn(fname, fmls', subst body (Env.bindAll fmls (map (fun v -> Var v) fmls') env))
  ...
  let rec desugar sexp = 
    let sexp' = desugarRules sexp in 
	  ...
	  Seq (Sym "funrec" :: bodyx :: defsx) -> 
	    Seq (Sym "funrec" :: desugar bodyx :: (map desugarDef defsx))
  ...
  and desugarDef sexp = 
    match sexp with 
      Seq[Sym "def"; Seq nameAndFmlxs; bodyx] -> Seq[Sym "def"; Seq nameAndFmlxs; desugar bodyx]
    | _ -> raise (SyntaxError("malformed def: " ^ (sexpToString sexp)))
  ...
  and sexpToPgm sexp = 
    match sexp with 
      (* Handle HOFL & FOFL programs that happen to have the restricted FOBS form *)
      Seq ((Sym "fobs" | Sym "fofl" | Sym "hofl") :: Seq formals :: bodyx :: declxs) -> 
	Pgm(map symToString formals, 
	    Funrec(sexpToExp bodyx,
    ...
    | Seq [Sym "intex"; Sexp.Int n; bodyx] -> 
            Pgm(map 
                  ...
                  (ListUtils.range 1 n),
                sexpToExp bodyx)
    | _ -> raise (SyntaxError ("invalid Fobs program: " ^ (sexpToString sexp)))
  ...
  and sexpToExp' sexp = 
    ...
    | Seq (Sym "funrec" :: bodyx :: fdeclxs) -> 
	Funrec(sexpToExp bodyx, flatten (map declToFcns fdeclxs))
    ...
    | _ ->  raise (SyntaxError ("invalid Fobs expression: "  ^ (sexpToString sexp)))
  ...
  and expToSexp e = 
    ...
    | Funrec(body,fcns) -> 
	Seq (Sym "funrec" :: expToSexp body :: map fcnToSexp fcns)
