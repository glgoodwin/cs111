(* Environment model interpreter for call-by-name Hoilic in which variables
   are modeled by OCAML refs containing thunks. *)

module HoilicCBNEnvInterp = struct
  ...
  (* Convert a value to a trivial thunk *)
  and delayedValu v = (fun () -> v)
  ...
  and eval exp env = 
    ...
    | App(rator,rand) ->  (* force left to right evaluation of rator and rand *)
        ...
        let argThunk = (fun () -> eval rand env) in 
	apply fcn argThunk
    | Assign(name,rhs) -> 
  ...
  and apply fcn argThunk = 
    match fcn with 
      Fun(fml,body,env) -> eval body (Env.bind fml (ref argThunk) env)
