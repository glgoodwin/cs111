(***************************************************************
 Syntax for Call-by-lazy HOILIC. This is like call-by-name HOILIC
 except that promises are used in place of thunks. (This difference
 could be abstracted over, but I have not done that here.)
 ****************************************************************)

module HoilicCBL = struct
  ...
  and exp =
    ...
    | Sequence of exp * exp (* sequential evaluation  *)
  ...
  and valu = 
    ...
    | Fun of var * exp * valu promise ref Env.env (* implicit cell holds promises *)

  and 'a thunk = unit -> 'a

  and 'a delayedList = DNil | DCons of 'a promise * 'a delayedList promise

  and primop = Primop of var * (valu list -> valu)

  let force = Promise.force
  ...
  let primops = [ 
    ...
    Primop("explode", checkOneArg checkString 
	     (fun s -> List (let rec loop i chars = 
                               if i < 0 then chars
                               else loop (i-1) (DCons (makePromise (fun () -> Char (String.get s i)), 
                                                       makePromise (fun () -> chars)))
