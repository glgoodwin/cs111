(* Environment model interpreter for Hofl in which all values are bound to thunks *)

module HoflEnvInterp = struct
  ...
  let rec run (Pgm(fmls,body)) ints = 
    ...
    in 
      if flen = ilen then 
        eval body (Env.make fmls (map (fun i -> fun () -> Int i) ints))
      else 
        raise (EvalError ("Program expected " ^ (string_of_int flen)
                          ^ " arguments but got " ^ (string_of_int ilen)))

  (* val eval : Hofl.exp -> (unit -> valu) Env.env -> valu *)
  and eval exp env = 
    match exp with 
      Lit v -> v
    | Var name -> 
    ...
    (* "Complex" environment rule for BINDREC that 
        (1) thunks environment values to perform knot-tying and
	...
	eval body
	  (let rec newEnvFcn = 
                     fun name -> 
 		       (Env.toFun (Env.bindAll names
				      (map (fun defn -> (fun () -> eval defn (Env.fromFun newEnvFcn)))
					 defns)
				      env))
		       name
  ...
  and apply fcn arg = 
    match fcn with 
      Fun(fml,body,env) -> eval body (Env.bind fml (fun () -> arg) env)
