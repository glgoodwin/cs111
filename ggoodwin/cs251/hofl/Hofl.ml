...
(* still to do: 
   + change read_line() in other repls to readSexp
   + abstract over repl
   + better repl (with top-level definitions and ability to read more than one line)
 *)
 ****************************************************************)
...
module Hofl = struct
  ...
  and valu = 
    ...
    | Symbol of string
    | List of valu list
    | Fun of var * exp * valu Env.env  (* Env needed only in env model, not subst model *)
  ...
  let primops = [ 
    ...
    Primop("fileToString", checkOneArg checkString (fun s -> String(File.fileToString s)));
    Primop("stringToSexp", checkOneArg checkString 
	     (let rec sexpToHoflList sexp = 
	       match sexp with 
		 Sexp.Int i -> Int i
	       | Sexp.Chr c -> Char c
	       | Sexp.Str s -> String s
	       ...
	       | Sexp.Sym "#e" -> List []
	       | Sexp.Sym s -> Symbol s
	       | Sexp.Flt f -> (raise (SyntaxError "Hofl doesn't allow floats"))
	       | Sexp.Seq eltxs -> List (List.map sexpToHoflList eltxs)
	     in fun str -> sexpToHoflList(Sexp.stringToSexp str)));
    ...
    Primop("error", checkTwoArgs (checkString,checkAny)
                      (fun s x -> raise (EvalError ("Hofl Error -- " ^ s ^ ":" ^ (valuToString x))))); 
  ...
  and isSpecial s = 
    List.mem s ["if"; "abs"; "&&"; "||"; "fun"; "bind"; "bindseq"; "bindpar"; 
  ...
  and sexpToPgm sexp = 
    ...
    | Seq [Sym "intex"; Sexp.Int n; bodyx] -> 
            Pgm(map 
                  ...
                  (ListUtils.range 1 n),
                sexpToExp bodyx)
    | _ -> raise (SyntaxError ("invalid Hofl program: " ^ (sexpToString sexp)))
  ...
  and sexpToExp' sexp = 
    ...
    | _ ->  raise (SyntaxError ("invalid Hofl expression: "  
                                ^ (sexpToString sexp)))
  ...
  and parseBindings bindingxs = 
    unzip (map (fun bindingx -> 
		 ...
		 | _ -> raise (SyntaxError ("ill-formed Hofl binding"
  ...
  and pgmToSexp p = 
    match p with 
      Pgm (fmls, e) -> 
        Seq [Sym "hofl"; Seq(map (fun s -> Sym s) fmls); expToSexp e]
