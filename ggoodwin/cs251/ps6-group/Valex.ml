...
module Valex = struct
  ...
  let primops = [ 
    ...
    (***********************************************************************)
    (* Add your PS6 Problem 3 primops here *)
    Primop ("abs", checkOneArg checkInt (fun x -> Int(abs x)));

    Primop ("sqrt", checkOneArg checkInt (fun x -> Int(int_of_float (sqrt (float x)))));

    Primop ("between", checkTwoArgs (checkInt, checkInt) (fun x y -> List(map (fun z -> Int(z)) (range x y)))); 

    Primop ("reverse", checkOneArg checkList (fun xs -> List(rev xs)));
  ...
  and desugarRules sexp = 
    ...
    (***********************************************************************)
    (* Add your PS6 Problem 4 desugaring rules for classify here *)
    | Seq [Sym "classify"; Sym v; Seq[Sym "otherwise"; default]] ->
	default 
    | Seq (Sym "classify" :: Sym v :: Seq[Seq[lo; hi]; body] ::rules) ->
	Seq [Sym "if"; Seq [Sym "&&"; Seq[Sym "<="; lo; Sym v]; Seq[Sym "<="; Sym v; hi]]; body; Seq(Sym "classify":: Sym v :: rules)]
    | Seq(Sym "classify" :: disc :: rules) ->
	let varFresh = StringUtils.fresh(var) in 
	(Seq[Sym "bind"; Sym varFresh; disc; Seq(Sym "classify" :: Sym varFresh :: rules)])
