module HoflTestEntries = struct
  ...
  let rec translateValexEntries entries = 
  ...
  let entries = (translateValexEntries ValexTestEntries.entries) @ hoflEntries

end



module type HOFL_INTERP = sig
  val run : Hofl.pgm -> int list -> Hofl.valu
end

module type HOFL_TEST = sig
  val test : unit -> unit
end
