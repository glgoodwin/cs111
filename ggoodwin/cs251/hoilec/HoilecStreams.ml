(***************************************************************
 A version of HOILEC that supports (1) promises and  
  (2) Scheme-like streams = tail-lazy lists with the following interface: 

     (sempty): create an empty stream
     (sempty? s): is s an empty stream?
     (sprep v s): prepend v onto (delayed) stream s
     (shead s): head of the empty stream
     (stail s): tail of the empty stream

  These are implemented as sugar/standard-definitions: 
  ...
  This version of Hoilec also supports floating-point numbers
  to allow encoding the the Newton-Rhapson method from 
  Hughes's article "Why functional programming matters".
 ****************************************************************)

module HoilecStreams = struct
  ...
  and valu = 
    ...
    | Cell of valu ref  
    | Float of float (* New in HoilecStreams *)
  ...
  let checkFloat v f = 
    match v with 
      Float fl -> f fl
    | _  -> raise (EvalError ("Expected a float but got: " ^ (valuToString v)))
  ...
  let farithop f = checkTwoArgs (checkFloat,checkFloat) (fun fl1 fl2 -> Float(f fl1 fl2))
  let frelop f = checkTwoArgs (checkFloat,checkFloat) (fun i1 i2 -> Bool(f i1 i2))
  ...
  let primops = [ 
    ...
    Primop("f-", farithop (-.));
    Primop("f*", farithop ( *. ));
    Primop("f/", farithop (fun x y -> 
                            if (y = 0.0) then 
                              raise (EvalError ("Division by 0.0: " 
			 		       ^ (string_of_float x)))
                            else x/.y)); 
  ...
  and desugarRules sexp = 
    ...
    (* DELAY sugar is new in HoilecStreams *)
    (* (delay E) ~~desugars-to~~> (make-promise (fun () E)) *)
    | Seq [Sym "delay"; bodyx] -> Seq [Sym "make-promise"; Seq [Sym "fun"; Seq []; bodyx]]

    (* SPREP sugar is new in HoilecStreams *)
    (* (sprep E1 E2) ~~desugars-to~~> (list E1 (delay E2)) *)
    | Seq [Sym "sprep"; headx; tailx] -> Seq [Sym "list"; headx; Seq [Sym "delay"; tailx]]
  ...
  and sexpToPgm sexp = 
	...
	let (declNames,declExpxs) = 
            ListUtils.unzip (flatten (map declToBindings 
                                          (* Include standard definitions *)
                                   	  ((stringToSexp "(load \"../hoilec/standard.hec\")") :: declxs)))
