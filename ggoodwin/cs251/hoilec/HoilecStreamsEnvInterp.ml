(**********************************************************************
   Environment model interpreter for HoilecStreams. 
   This is like HoilecEnvInterp except that it loads the declarations
   in "standard.hec" into the top-level repl. 
 **********************************************************************)

module HoilecStreamsEnvInterp = struct
  ...
  (* val eval : HoilecStreams.exp -> HoilecStreams.valu Env.env -> HoilecStreams.valu *)
  and eval exp env = 
    match exp with 
      Lit v -> v
    | Var name -> 
  ...
  let rec repl () =
    ...
    (* Add all definitions from stanard.hec into global environment *)
    let globalBindings = map (fun (name,defnx) -> (name, sexpToExp defnx))
  	                     (declToBindings (Sexp.stringToSexp "(load \"../hoilec/standard.hec\")")) in 
    let _ = for_each (fun (name,defn) -> Env.define name (eval defn globalEnv) globalEnv)
                     globalBindings in
    let rec loop () = 
      let _ = print "\nhoilec-streams> " in 
