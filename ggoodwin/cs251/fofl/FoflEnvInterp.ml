...
module FoflEnvInterp = struct
  ...
  let rec run scope (Pgm(fmls,body,fcns)) ints = 
    ...
    in if flen <> ilen then 
		...
		Lit v -> v
	      ...
	      | App(fname, rands) -> 
		  apply fname (map (flip eval venv) rands) venv
		...
		None -> raise (EvalError ("unknown function " ^ fname))
		  ...
		  let flen = length formals
		  and alen = length actuals
		  in if flen <> alen then 
		    raise (EvalError ("Function " ^ name ^ " expected " ^ (string_of_int flen)
                            ^ " arguments but got " ^ (string_of_int alen)))
		  else 
  ...
  (* A function for running a program in a file *)
