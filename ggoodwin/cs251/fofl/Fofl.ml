(***************************************************************
 FOFL is a first-order functional language. 
 All functions are declared at top-level in a separate namespace. 
 ****************************************************************)
...
module Fofl = struct
  ...
  and sexpToPgm sexp = 
    match sexp with 
      Seq (Sym "fofl" :: Seq formals :: bodyx :: declxs) -> 
  ...
  and expToSexp e = 
    ...
    | Bind(name,defn,body) -> Seq [Sym "bind"; Sym name; expToSexp defn; expToSexp body]
